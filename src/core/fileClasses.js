			...
			return Promise.all(testPromises)
				.then(function(variableClassification){
					...
					categoryInfo.supportedCategories.forEach(function(category){
						...
						let categoryVariables = obj.content.variables.filter(d=>d.category == category)
						
						obj.content.header[categoryInfo.cat2prop[category]] = categoryVariables.map(d=>d.name)
					})
	...
	export class contour2dFile extends onDemandFile {
		...
		static structure = {
			...
			json2contour2dFile: function(content){
				...
				var surfaces = Array.isArray(content.surfaces) ? content.surfaces : [content.surfaces]
				
				// First perform a mapping?? And then test??
		...
		static test = {
			...
			surface: function(surface){
				...
				return surface.variables.length > 0
				
			}, // surface
			
			variable: function(data, varname){
				
				// Find all possible variables. The variables are deemed available if they are the same length as the x and y arrays. Also, they must contain only numeric values.
				let compulsory = ["x", "y", "size"]
				
				let vals = data[varname]
				
				let L_ = undefined
				if( !compulsory.includes(varname) &&
				    Array.isArray( vals ) && 
					!vals.some(isNaN) ){
